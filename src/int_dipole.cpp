...
bool operator<(int_dipole const& a, int_dipole const& b)
{
    ...
    return std::make_tuple(a.emitter(), a.spectator(), a.splitting()) <
        std::make_tuple(b.emitter(), b.spectator(), b.splitting());
}

bool operator==(int_dipole const& a, int_dipole const& b)
{
    return (a.splitting() == b.splitting()) && (a.type() == b.type()) &&
...
{
    // TODO: take `splitting()` into account
