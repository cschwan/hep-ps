...
public:
    ...
    /// Evaluates all densities for the previously generated phase space point and writes them into
    /// `densities`. This vector must have the size given by \ref channels. The return value is an
    /// additional jacobian.
    virtual T densities(std::vector<T>& densities) = 0;

    /// Returns how many random numbers are needed to construct a phase space point.
    virtual std::size_t dimensions() const = 0;

    /// Uses `random_numbers` to construct a phase space point which is written as a set of
    /// four-vectors into `momenta` using the specified `channel` for the given center-of-mass frame
    /// energy `cmf_energy`.
    ...
    /// Returns an object of class \ref luminosity_info that contains the energy of the previously
    /// generated phase space point.
    virtual luminosity_info<T> info() const = 0;

    /// Returns the size of the vector `momenta` that has to be passed to \ref generate.
