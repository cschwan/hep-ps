...
/// Class that represents a single phase space point in the center-of-mass frame (CMF) of the
/// (partonic) collision. The member functions of this class enable one to calculate kinematic
/// observables in the LAB frame.
template <typename T>
class phase_space_point
{
public:
    /// Constructor. The phase space point in `phase_space` must be given in the CM frame. The
    /// parameter `rapidity_shift` enables this class to boost into the LAB frame.
    phase_space_point(
