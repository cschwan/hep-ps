...
template <typename T>
class test_int_dipoles_class
{
public:
    test_int_dipoles_class(std::size_t alphas_power, T alphas)
    ...
    void insertion_terms(
        hep::int_dipole const& term,
        nonstd::span<hep::scales<T> const> scales,
        std::vector<T> const& /*phase_space*/,
        T /*x*/,
        T eta,
        std::vector<hep::ab_term<T>>& results
    ) const {
        results.clear();
    ...
    std::vector<hep::int_dipole> dipoles() const
    {
        return { hep::int_dipole{0, hep::dipole_vertex(4, 4, 22)},
            hep::int_dipole{1, hep::dipole_vertex(2, 2, 22)} };
    }
...
template <typename T>
class test_int_dipoles_scale_setter
{
public:
    test_int_dipoles_scale_setter(bool dynamic_scale)
...
void test_int_dipoles_integrand(
    hep::initial_state_set set,
    ...
    test_int_dipoles_class<T> int_dipoles{alphas_power, alphas};
    test_int_dipoles_scale_setter<T> scale_setter{dynamic_scale};

    // born gets copied, therefore `global_scales` must be global
    auto integrand = hep::make_int_dipoles_integrand<T>(
        int_dipoles,
        int_dipoles,
...
TEST_CASE("integrated dipoles integrand static scale", "[int_dipoles_integrand]")
{
    // choose an initial state that doesn't have a symmetry factor
    hep::initial_state_set set{hep::initial_state::cq_uq};

    test_int_dipoles_integrand(set, 0, T(10.0), false);
    test_int_dipoles_integrand(set, 1, T(10.0), false);
    test_int_dipoles_integrand(set, 2, T(10.0), false);
    test_int_dipoles_integrand(set, 3, T(10.0), false);
    test_int_dipoles_integrand(set, 4, T(10.0), false);
}

TEST_CASE("integrated dipoles integrand dynamic scale", "[int_dipoles_integrand]")
{
    // choose an initial state that doesn't have a symmetry factor
    hep::initial_state_set set{hep::initial_state::cq_uq};

    test_int_dipoles_integrand(set, 0, T(10.0), true);
    test_int_dipoles_integrand(set, 1, T(10.0), true);
    test_int_dipoles_integrand(set, 2, T(10.0), true);
    test_int_dipoles_integrand(set, 3, T(10.0), true);
    test_int_dipoles_integrand(set, 4, T(10.0), true);
}
